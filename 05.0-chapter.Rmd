# Eesti elukeskkond aastal 2050 {#chapter50 .chapter5_section .intro_section}

## Eesti elukeskkond aastal 2050 {-.chapter_section .chapter5_section}
...
fig54_data$variable <- factor(fig54_data$variable, levels = c("Parkimiskohad","Tänavavõrk","Jalakäijate ruum","Eluruumid","Bürooruumid","Kaubandus- ja teenindusruumid","Meelelahutus-, haridus-, tervishoiu- ja muud avalikud ruumid","Tööstus- ja laoruumid"))

  fig54_plot <- ggplot(data = fig54_data) +
    geom_col(mapping = aes(x=variable, y = as.numeric(as.character(value)), fill=variable), size=0.7) +
    facet_wrap(~Linnaosa, nrow=8, strip.position="left") +
    coord_flip() +
    labs(y="m²", x="") +
    scale_y_continuous(labels = comma, limits=c(0,7600000), breaks=c(0, 2500000,5000000,7500000)) +
    scale_fill_manual(values=c("#500e0c", "#a01b17", "#a8aca6", "#86858a","#79ac2c","#3d5616", "#c67674", "#c8c6bf"),guide = guide_legend(reverse = TRUE)) +
