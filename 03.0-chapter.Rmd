...
```{r, figure301, fig.cap='Inimarengu indeksi ja ökosüsteemide elujõulisuse indeksi seos 179 riigi põhjal ning Eesti positsiooni (punased punktid, aastad) muutus 2005–2014.', out.width='80%', fig.align='center', echo=FALSE, message=FALSE}
...
library("tidyverse")
 ...
 fig301_data_est <- subset (fig301_data, country_name == "Estonia", select=c(iso3,country_name,hdi_2014, hdi_2017, country,epi_current, eco_current))
  ...
  fig301_plot <- ggplot() +
    geom_point(data = fig301_data, mapping = aes(x=hdi_2014, y=eco_current), colour="blue", show.legend = NA) +
    
    geom_smooth(data = fig301_data, mapping = aes(x=hdi_2014, y=eco_current),method=lm, se=FALSE, colour="black") +
    
    geom_point(data = fig301_data_est, mapping = aes(x=hdi_2014, y=eco_current), colour="red", show.legend = NA) +
    geom_text(data = fig301_data_est, mapping = aes(x=hdi_2014, y=eco_current, label="2014"),hjust=1, vjust=0, colour="red")+
    
    geom_point(data = fig301_data_est, mapping = aes(x=0.8, y=65), colour="red", show.legend = NA) +
      geom_text(data = fig301_data_est, mapping = aes(x=0.8, y=65, label="???"),hjust=0, vjust=0, colour="red") +
...
```{r, figure302, fig.cap='Metsamaa, sh, riigimetsa (eristatud puistud ja metsata metsamaa) osakaal maakondade üldpindalast 2017. aastal (andmed: Raudsaar jt, 2018)', out.width='80%', fig.align='center', echo=FALSE, message=FALSE}
...
library("reshape")
 ...
 fig302_data_melt <- melt (fig302_data_clean, id=c("county"))
  ...
  fig302_plot <- ggplot() +
    geom_bar(data = fig302_data_melt, mapping = aes(x=county, y=value*100, fill=variable), width=0.7, stat="identity") +
    
    labs(y="% pindalast", x="") 
  
  print(fig302_plot + fig_theme_general)
