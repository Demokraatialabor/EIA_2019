...
```{r, figure122, fig.cap='Viljandi-, Valga-, Võru- ja Põlvamaa elanike töökohtade paiknemine elukoha maakonnas, mujal Eestis või väljaspool Eestit 2000. ja 2011.aastal', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE}

 library("tidyverse")
library("svglite")

  fig122 <- read.csv ("data/23_fig2_data.csv", header=TRUE, fileEncoding="utf-8")
  fig2plot <- ggplot(data = fig122) +
  geom_col(mapping = aes(x=county, y=value, fill=location_est), width=0.7) +
  facet_wrap(~year, nrow=1) +
  labs(y="Töökohtade arv", x="", fill="Töökoha paiknemine") +
  scale_fill_manual(values=c("#482683", "#79AC2C", "#369B9E"))
  
 fig2theme <- theme(
                    text = element_text(color = "#777777", size = 13),
                    ...
                    strip.text = element_text(colour = "#777777", size = 14),
                 )
  
  print(fig2plot + fig2theme)

  ggsave(path="exported_figures/SVG/", filename="fig122.svg", height=150, width=200, units="mm")
  ggsave(path="exported_figures/PDF",filename="fig122.pdf", height=150, width=200, units="mm")
  ggsave(path="exported_figures/PNG",filename="fig122.png",height=150, width=200, units="mm")
```
```{block, type='imgsource'}
Allikas: Statistikaamet
```
...
```{r, joonis123, fig.cap='Kes kujundavad maapiirkondade kuvandit Eesti trükimeedias?', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE}

library("tidyverse")
  
 fig3 <- read.csv ("data/23_fig3_data.csv", header=TRUE, fileEncoding="utf-8")
  ...
  fig3plot <- ggplot(data = fig3) +
    ...
    geom_text(aes(label = proportion, x=body_est, y=proportion), nudge_x = 0, nudge_y = 3, color="#777777") +
    labs(y="%", x="") + 
    coord_flip ()
  
  fig3theme <- theme(aspect.ratio = 0.8,
                     text = element_text(color = "#777777", size = 14),
                     panel.background = element_rect(fill = "white", colour = "white"),
                     panel.grid.major = element_line(colour="#cccccc", size = (0.2)),
                     panel.grid.minor = element_line(size = (0.2), colour="#cccccc"),
                     axis.line = element_line(size = 0.2, colour = "#cccccc"),
...
``` {block, type='imgsource'}
Allikas: Plüschke-Altof (2017). Arvutused põhinevad 296 arvamusartiklil (Postimees, Eesti Päevaleht, Maaleht 2011–2015).
```
...
```{r, joonis124, fig.cap='Mis kohti kirjeldatakse Eesti trükimeedias kui ääremaad?', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE}
library(ggwordcloud)
library(dplyr)

fig4 <- read.csv ("data/23_fig4_data.csv", header=TRUE)
fig4 <- sample_frac(fig4, 1L) ##randomizing order of data frame to keep the words shuffled

set.seed(42)
fig4plot <- ggplot(fig4, aes(label = location_est, size = freq, color=freq)) +
  geom_text_wordcloud_area(area_corr_power = 1/.9, eccentricity = 0.4) +
