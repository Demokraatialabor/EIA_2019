## Planeerimisvabaduse võidukäik eeslinna-Eestis {#chapter13 .chapter_section .chapter1_section}
...
```{block, type='points'}
* Krundiviisiline eeslinna keskkonna arendamine loob hõreda, vähesidusa, hajusalt kujundatud avaliku ruumi ja killustatud asustuse.
```
...
library("reshape")
 ...
 fig133_data_clean$area <- factor (fig133_data_clean$area, levels = c("Tallinn","Tallinna eeslinn", "Tartu","Tartu eeslinn","Pärnu","Pärnu eeslinn"))
  ...
  fig133_plot <- ggplot(fig133_data_clean) +
    ...
    expand_limits (y=0) +
    scale_colour_manual(values=c("#482683","#3A404A","#917DB5","#A8ACA6","#DAD4E6","#E6E3DA"))
...
Linnavormiliselt eristuvad põhiliselt ühepereelamutega hoonestatud asumid (tihedusega 200 eluruumi/km2 ehk 600 inimest/km2) ning rida- ja korterelamuhoonestus (asustustihedus 600‒800 eluruumi/km2 ehk kuni 2000 inimest/km2) ([joonis 1.3.4](#figure134)). Hõredamas ja maalisema maastikumustriga eeslinnas langeb rahvastiku tihedus 50‒100 inimeseni/km2-l. Lisaks elamualadele laiuvad eeslinnas äri- ja logistikakeskused. Sotsiaalmaad pigem pole.

<p class="caption" id="figure134"><span class="figure-number">Joonis 1.3.4.</span> Eeslinnamorfoloogia Vahi alevikus Tartu vallas</p>

```{r, figure134, out.width='90%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE, warnings=FALSE}
...
```{r, figure135, out.width='90%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE, warnings=FALSE}
...
```{block, type='blockquote-right'}
Tänapäeval on tavapärane, et ees- linnastunud ruumi loomise aluseks on üksikud detailplaneeringud, mis tühistavad suuresti omavalitsuste üldplaneeringute tervikliku ja sidu- sa asustuse arendamise plaani.
```
...
Tegelikult määratlevad eeslinnaasustuse arengu väikese pindalalised, killustatult ja juhuslikus ajalises järgnevuses koostatud ja kehtestatud detailplaneeringud, mida ilmestab Rae ja Raasiku valla planeeringukaart ([joonis 1.3.6](#figure136)). 
...
```{block, type='blockquote-left'}
Arendatava-ehitamata maa ülepakkumine viib juhuslikes külgnevustes ja arendusjärjestuses ebaloogilise katkestustega maa- kasutusmustrini.
```
...
```{r, figure138, out.width='90%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE, warnings=FALSE}
...
```{block, type='blockquote-right'}
Nüüdisaegne tankla ei kauple ainult autokütusega, vaid täidab parema puudumisel laiaotstarbe- liselt toidupoe, kohviku, kiirsöökla, puhkekoha ja isegi suitsunurga ülesandeid.
```
...
```{r, figure139, out.width='90%', fig.asp=.75, fig.align='center', echo=FALSE, message=FALSE, warnings=FALSE}
...
```{block, type='blockquote-right'}
Eeslinnakasvu ruumilised taga- järjed kuhjuvad erahuviliste toimijate ‒ olgu need siis kinnis- varaarendajad või noored pered kodu- ostjatena ‒ tegutsemise juhustena ega ole raamistatud avalikest huvidest.
```
...
<p id=”Randolph2012”>Randolph, J. 2012. Environmental Land Use Planning and Management. Washington: Island Press, 746.</p>
